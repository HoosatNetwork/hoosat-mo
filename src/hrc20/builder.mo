/// HRC20 Transaction Builder
///
/// This module provides high-level functions for building HRC20 commit-reveal
...
module {
  ...
  public type CommitRevealPair = {
    commitTx: Types.HoosatTransaction;       // Transaction to broadcast first
  ...
  /// Build a commit transaction for a HRC20 operation
  ///
  ...
  /// HRC20 operation data.
  ///
  /// @param pubkey - Public key bytes (32 for Schnorr, 33 for ECDSA)
  /// @param operation_json - JSON string of the HRC20 operation
  ...
  public func buildCommit(
    ...
    change_address: Text,
    use_ecdsa: Bool
  ) : Result.Result<CommitRevealPair, Errors.HoosatError> {
    ...
    let changeScriptResult = Address.decodeAddress(change_address, null);
  ...
  ///
  /// This creates the second transaction that reveals the HRC20 operation
  ...
  public func buildRevealOutputs(
    ...
    amount: Nat64
  ) : Result.Result<[Types.TransactionOutput], Errors.HoosatError> {
    // Decode recipient address to get scriptPublicKey
    let decoded = Address.decodeAddress(recipient_address, null);
  ...
  /// @param prefix - Address prefix (e.g., "hoosat" for mainnet)
  /// @return Hoosat address or error
  public func getP2SHAddress(
    addressType: Nat,
    scriptHash: [Nat8],
    prefix: Text
  ) : Result.Result<Text, Errors.HoosatError> {
    // For P2SH, addressType should be 2
    assert(addressType == 2);

    // generateAddress takes (pubkey: Blob, addr_type: Nat, prefix: ?Text)
    let result = Address.generateAddress(Blob.fromArray(scriptHash), addressType, ?prefix);
    switch (result) {
  ...
  /// Estimate fee for a HRC20 operation
  ///
  ...
  /// Minimum amount for HRC20 operations
  /// Deploy requires 1000 HTN fee, so we need more than that
  public let MIN_COMMIT_AMOUNT: Nat64 = 100_100_000_000;  // 1001 HTN (1000 fee + 1 HTN output)

  /// Recommended commit amount for HRC20 operations
  /// This covers the 1000 HTN deploy fee plus buffer for output
  public let RECOMMENDED_COMMIT_AMOUNT: Nat64 = 100_100_000_000;  // 1001 HTN

  /// Deploy fee required by HRC20 protocol
  public let DEPLOY_FEE: Nat64 = 100_000_000_000;  // 1000 HTN
};
