...
persistent actor {
    ...
    public func runTests() : async Text {
        Debug.print("🧪 Running HRC20 Integration Tests...");
        ...
        total += 1;
        Debug.print("\n🔧 Test 1: Generate valid Hoosat address");

        let test_pubkey = Array.freeze(Array.init<Nat8>(32, 0xAA));  // 32-byte Schnorr pubkey
        let test_addr_result = Address.generateAddress(Blob.fromArray(test_pubkey), 0, null);  // Schnorr
        ...
        let envelope = ScriptBuilder.buildDataEnvelope("hooplex", json_bytes, []);

        // Check envelope contains the protocol identifier
        let envelope_hex = ScriptBuilder.bytesToHex(envelope);
        let contains_hooplex = Text.contains(envelope_hex, #text "686f6f70");  // "hoop" in hex
        ...
        let (commit_fee, reveal_fee) = HRC20Builder.estimateFees(operation_json);
        ...
        // Test 4: P2SH address generation
        total += 1;
        Debug.print("\n🔧 Test 4: P2SH address generation");

        let script_hash = Array.freeze(Array.init<Nat8>(32, 0xAA));
        let p2sh_address = HRC20Builder.getP2SHAddress(2, script_hash, "hoosat");

        switch (p2sh_address) {
            case (#ok(addr)) {
                let has_prefix = Text.startsWith(addr, #text "hoosat:");
                if (has_prefix) {
                    Debug.print("  ✓ P2SH address generated: " # addr);
                    passed += 1;
                } else {
                    Debug.print("  ✗ Address missing hoosat: prefix");
                };
            };
            case (#err(e)) {
                Debug.print("  ✗ Failed to generate address: " # debug_show(e));
        ...
        // Test 5: Reveal outputs builder
        total += 1;
        Debug.print("\n🔧 Test 5: Build reveal outputs");

        let reveal_outputs = HRC20Builder.buildRevealOutputs(
            test_address,
        ...
        // Test 6: Mint operation format
        total += 1;
        Debug.print("\n🔧 Test 6: Format mint operation");
        ...
        // Test 7: Transfer operation format
        total += 1;
        Debug.print("\n🔧 Test 7: Format transfer operation");
        ...
        if (passed == total) {
            Debug.print("🎉 All HRC20 integration tests passed!");
            "All tests passed ✅"
        } else {
